...
// Converts ResolvedGroupingSet to an int64_t grouping set id.
...
// Converts ResolvedCube to a list of int64_t grouping set ids.
...
// Converts ResolvedRollup to a list of int64_t grouping set ids.
...
Algebrizer::AlgebrizeFunctionCallWithLambda(
  ...
  for (const auto& arg : function_call->generic_argument_list()) {
    if (arg->expr() != nullptr) {
      ...
      args.push_back(std::make_unique<InlineLambdaArg>(std::move(lambda)));
...
absl::StatusOr<std::unique_ptr<ValueExpr>> Algebrizer::AlgebrizeIsError(
  ...
  ZETASQL_ASSIGN_OR_RETURN(std::unique_ptr<ValueExpr> is_error_expr,
                   IsErrorExpr::Create(std::move(args[0])));
...
absl::StatusOr<std::unique_ptr<RelationalOp>> Algebrizer::AlgebrizeUdaCall(
  ...
  ParameterMap column_map;
  SystemVariablesAlgebrizerMap system_variables_map;
  Algebrizer uda_algebrizer(language_options, algebrizer_options, type_factory,
  ...
  for (int i = 0; i < aggregate_exprs.size(); ++i) {
    const VariableId agg_variable_name =
        uda_algebrizer.column_to_variable_->AssignNewVariableToColumn(
    ...
    ZETASQL_ASSIGN_OR_RETURN(std::unique_ptr<AggregateArg> agg,
                     uda_algebrizer.AlgebrizeAggregateFn(
                         agg_variable_name, anonymization_opts,
                         /*filter=*/nullptr, aggregate_exprs[i]));
    algebrized_aggregate_exprs.push_back(std::move(agg));
  }

  // Construct an AggregateOp that computes all the aggregates referenced in
  // the UDA. The input is a table aware of the UDA argument names that
  // represents the grouped rows on which to perform the aggregation.
  std::unique_ptr<RelationalOp> input =
      RowsForUdaOp::Create(std::move(argument_names));
  ...
  ZETASQL_ASSIGN_OR_RETURN(std::unique_ptr<ValueExpr> algebrized_tree,
                   uda_algebrizer.AlgebrizeExpression(&function_expr));
...
Algebrizer::CreateTemplatedUserDefinedAggregateFn(
  ...
  for (const auto& arg : aggregate_function->signature().arguments()) {
    argument_is_aggregate.push_back(!arg.options().is_not_aggregate());
  }
  ...
  AggregateFunctionEvaluatorFactory AggregateFn =
      [signature, anonymization_options, function_expr, aggregate_exprs,
       aggregate_expr_columns, argument_names, argument_is_aggregate,
       language_options, algebrizer_options, type_factory,
       this](const FunctionSignature& sig)
      -> absl::StatusOr<std::unique_ptr<AggregateFunctionEvaluator>> {
    ZETASQL_ASSIGN_OR_RETURN(
        std::unique_ptr<RelationalOp> algebrized_uda,
        AlgebrizeUdaCall(anonymization_options, *function_expr, aggregate_exprs,
                         aggregate_expr_columns, argument_names,
                         language_options, algebrizer_options, type_factory));

    return MakeUserDefinedAggregateFunctionEvaluator(
        std::move(algebrized_uda), std::move(argument_names),
...
Algebrizer::CreateNonTemplatedUserDefinedAggregateFn(
  ...
  std::vector<std::string> argument_names = sql_function->GetArgumentNames();
  std::vector<const ResolvedExpr*> aggregate_exprs;
  ResolvedColumnList aggregate_expr_columns;
  for (const std::unique_ptr<const ResolvedComputedColumn>& agg_expr :
...
Algebrizer::CreateUserDefinedAggregateFn(
  ...
  if (aggregate_function->function()->Is<SQLFunction>()) {
    return CreateNonTemplatedUserDefinedAggregateFn(aggregate_function,
                                                    anonymization_options);
  }
  if (aggregate_function->function()->Is<TemplatedSQLFunction>()) {
    return CreateTemplatedUserDefinedAggregateFn(aggregate_function,
                                                 anonymization_options);
  }
  return ::zetasql_base::InvalidArgumentErrorBuilder()
  ...
  switch (kind) {
    case FunctionKind::kInvalid: {
      ...
      ZETASQL_ASSIGN_OR_RETURN(function,
                       CreateUserDefinedAggregateFn(aggregate_function,
  ...
  switch (expr->node_kind()) {
    ...
    case RESOLVED_ARGUMENT_REF: {
      ...
      if (argument_ref->argument_kind() == ResolvedArgumentDef::AGGREGATE) {
        ...
        ZETASQL_ASSIGN_OR_RETURN(
            val_op,
            DerefExpr::Create(VariableId(argument_ref->name()), expr->type()));
        break;
      } else {
...
absl::StatusOr<AnonymizationOptions> GetAnonymizationOptions(
  ...
  // Scale epsilon by max_groups_contributed (if specified).
  if (anonymization_options.max_groups_contributed.has_value() &&
      !anonymization_options.max_groups_contributed.value().is_null()) {
    anonymization_options.epsilon = Value::Double(
        anonymization_options.epsilon->double_value() /
        anonymization_options.max_groups_contributed->int64_value());
  }
...
absl::StatusOr<AnonymizationOptions> GetDifferentialPrivacyOptions(
  ...
  if (anonymization_options.epsilon->double_value() <= 0 ||
      !std::isfinite(anonymization_options.epsilon->double_value())) {
    return zetasql_base::InvalidArgumentErrorBuilder()
           << "Epsilon must be finite and positive, but is "
           << anonymization_options.epsilon->double_value();
  }
...
Algebrizer::AlgebrizeAggregateScanBase(
  ...
  for (const std::unique_ptr<const ResolvedComputedColumnBase>& agg_expr :
    ...
    ZETASQL_RET_CHECK(agg_expr->Is<ResolvedComputedColumn>());
    const ResolvedColumn& column =
  ...
  if (!aggregate_scan->grouping_set_list().empty()) {
    ...
    // Make sure we can use an int64_t to represent the grouping set id.
...
Algebrizer::AlgebrizeDifferentialPrivacyAggregateScan(
    const ResolvedDifferentialPrivacyAggregateScan* aggregate_scan) {
  ZETASQL_ASSIGN_OR_RETURN(AnonymizationOptions anonymization_options,
                   GetDifferentialPrivacyOptions(aggregate_scan));
  ZETASQL_ASSIGN_OR_RETURN(
      std::unique_ptr<AggregateOp> relation_op,
      AlgebrizeAggregateScanBase(aggregate_scan, anonymization_options));
...
absl::StatusOr<std::unique_ptr<RelationalOp>> Algebrizer::AlgebrizeScan(
  ...
  switch (scan->node_kind()) {
    ...
    case RESOLVED_DIFFERENTIAL_PRIVACY_AGGREGATE_SCAN: {
      ZETASQL_ASSIGN_OR_RETURN(
          rel_op, AlgebrizeDifferentialPrivacyAggregateScan(
    ...
    case RESOLVED_TVFSCAN: {
      ZETASQL_ASSIGN_OR_RETURN(rel_op,
                       AlgebrizeTvfScan(scan->GetAs<ResolvedTVFScan>()));
      break;
    }
    default:
      return ::zetasql_base::UnimplementedErrorBuilder()
...
std::unique_ptr<RootData> Algebrizer::GetRootData() {
  auto root_data = std::make_unique<RootData>();
...
absl::StatusOr<ProtoFieldRegistry*> Algebrizer::AddProtoFieldRegistry(
  ...
  auto registry = std::make_unique<ProtoFieldRegistry>(registry_id);
...
absl::StatusOr<ProtoFieldReader*> Algebrizer::AddProtoFieldReader(
  ...
  auto reader =
      std::make_unique<ProtoFieldReader>(reader_id, access_info, registry);
...
Algebrizer::AlgebrizeRecursiveScan(
  ...
  // TODO: Implement recursion depth modifier.
  if (recursive_scan->recursion_depth_modifier() != nullptr) {
    return absl::UnimplementedError(
        "WITH DEPTH modifier is not supported yet in reference implementation");
  }
...
absl::StatusOr<std::unique_ptr<RelationalOp>> Algebrizer::AlgebrizeTvfScan(
  ...
  ZETASQL_ASSIGN_OR_RETURN(
      std::unique_ptr<TVFOp> tvf_op,
      TVFOp::Create(tvf_scan->tvf(), std::move(arguments),
                    std::move(output_columns), std::move(variables),
                    tvf_scan->function_call_signature()));
  return std::move(tvf_op);
}
